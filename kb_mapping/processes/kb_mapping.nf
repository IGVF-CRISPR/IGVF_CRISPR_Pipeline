process mappingscRNA {
    ...
    path("ks_transcripts_out/counts_unfiltered/adata.h5ad"), emit: ks_transcripts_out_adata_h5ad

    script:
        """
        k_bin=\$(which kallisto)
        chemistry=\$(extract_parsed_seqspec.py --file ${parsed_seqSpec_file})
        kb count -i ${transcriptome_idx} -g ${transcriptome_t2g} --verbose -w ${whitelist} --h5ad --kallisto \$k_bin -x \$chemistry -o ks_transcripts_out -t ${task.cpus} ${fastq_file1} ${fastq_file2} --overwrite -m 48G
        """
}
